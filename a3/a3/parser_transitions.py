...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition =="S":
            ...
            self.dependencies.append((self.stack[-1], dependent))#添加键值对
        elif transition == "LA":
    ...
    def parse(self, transitions):
        ...
        @return dsependencies (list of string tuples): The list of dependencies produced when
                                                        ...
                                                        tuples where each tuple is of the form (head, dependent).
        """
        for transition in transitions:#通过神经网络训练出一系列的指令集，每一次根据当下的指令集进行三种文字操作之一
...
def minibatch_parse(sentences, model, batch_size):
    ...
    @param sentences (list of list of str): A list of sentences to be parsed#字符串列表的列表
                                            (each sentence is a list of words and each word is of type string)
    @param model (ParserModel): The model that makes parsing decisions. It is assumed to have a function
     #做出语段决定的函数            model.predict(partial_parses) that takes in a list of PartialParses as input and
