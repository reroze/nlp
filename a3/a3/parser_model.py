...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        @param n_features (int): number of input features#输入的特征层数
        @param hidden_size (int): number of hidden units#隐藏层
        ...
        self.embed_size = embeddings.shape[1]#向量维数
        ...
        self.embed_to_hidden=nn.Linear(self.n_features*self.embed_size, self.hidden_size)#构建线性层？ self.n_features*self.embed_size变量数乘以向量维数，所有数值的个数即为输入维度 后面的隐藏层个数是输出层维度
        nn.init.xavier_uniform_(self.embed_to_hidden.weight)#初始化权重
    ...
    def embedding_lookup(self, t):
        ...
        x = self.pretrained_embeddings(t)#将batch_size,n_features,embedding_size 转换成batch_size,n_features*embedding_size
        x = x.view(x.size()[0], -1)#保留第一项不变，剩下的一项自动分配
    ...
    def forward(self, t):
        """ Run the model forward.
            ...
            #不会在这里使用softmax函数，因为在计算loss函数的时候会包含softmax函数
            PyTorch Notes:
                - Every nn.Module object (PyTorch model) has a `forward` function.#前向传播
        ...
        embeddings = self.embedding_lookup(t)#reshape the vec
        h = self.embed_to_hidden(embeddings)
        nn.functional.relu_(h)
        h_dropout = self.fropout(h)
